...
框架提供的Table 列表组件基于 [vxe-table](https://vxetable.cn/v4/#/grid/api?apiKey=grid)，结合`Igourd Form 表单`进行了二次封装。

其中，表头的 **表单搜索** 部分采用了`Igourd Form表单`，表格主体部分使用了`vxe-grid`组件，支持表格的分页、排序、筛选等功能。
...
使用 `useIgourdVxeGrid` 创建最基础的表格。

<DemoPreview dir="demos/igourd-vxe-table/basic" />

## 远程加载

通过指定 `proxyConfig.ajax` 的 `query` 方法，可以实现远程加载数据。

<DemoPreview dir="demos/igourd-vxe-table/remote" />
...
<DemoPreview dir="demos/igourd-vxe-table/tree" />

## 固定表头/列

列固定可选参数： `'left' | 'right' | '' | null`

<DemoPreview dir="demos/igourd-vxe-table/fixed" />
...
<DemoPreview dir="demos/igourd-vxe-table/custom-cell" />

## 搜索表单

**表单搜索** 部分采用了`Igourd Form 表单`，参考 [Igourd Form 表单文档](/components/common-ui/igourd-form)。

当启用了表单搜索时，可以在toolbarConfig中配置`search`为`true`来让表格在工具栏区域显示一个搜索表单控制按钮。表格的所有以`form-`开头的命名插槽都会被传递给搜索表单。

### 定制分隔条

当你启用表单搜索时，在表单和表格之间会显示一个分隔条。这个分隔条使用了默认的组件背景色，并且横向贯穿整个Igourd Vxe Table在视觉上融入了页面的默认背景中。如果你在Igourd Vxe Table的外层包裹了一个不同背景色的容器（如将其放在一个Card内），默认的表单和表格之间的分隔条可能就显得格格不入了，下面的代码演示了如何定制这个分隔条。

```ts
const [Grid] = useIgourdVxeGrid({
  formOptions: {},
  gridOptions: {},
  // 完全移除分隔条
  separator: false,
...
<DemoPreview dir="demos/igourd-vxe-table/form" />

## 单元格编辑

通过指定`editConfig.mode`为`cell`，可以实现单元格编辑。

<DemoPreview dir="demos/igourd-vxe-table/edit-cell" />

## 行编辑

通过指定`editConfig.mode`为`row`，可以实现行编辑。

<DemoPreview dir="demos/igourd-vxe-table/edit-row" />
...
<DemoPreview dir="demos/igourd-vxe-table/virtual" />

## API

`useIgourdVxeGrid` 返回一个数组，第一个元素是表格组件，第二个元素是表格的方法。
...
useIgourdVxeGrid 返回的第二个参数，是一个对象，包含了一些表单的方法。
...
| formOptions | 表单参数 | `IgourdFormProps` | - |
