...
# API URL
VITE_GLOB_API_URL=https://mock-napi.igourd.pro/api
...
```ts
window._IGOURD_ADMIN_PRO_APP_CONF_ = {
  VITE_GLOB_API_URL: 'https://mock-napi.igourd.pro/api',
};
Object.freeze(window._IGOURD_ADMIN_PRO_APP_CONF_);
Object.defineProperty(window, '_IGOURD_ADMIN_PRO_APP_CONF_', {
  configurable: false,
  writable: false,
});
```
...
To access the variables inside `_app.config.js`, you need to use the `useAppConfig` method provided by `@igourd/hooks`.
...
- First, add the variable that needs to be dynamically configurable in the `.env` file or the corresponding development environment configuration file. The variable must start with `VITE_GLOB_*`, for example:
  ...
  ```bash
  VITE_GLOB_OTHER_API_URL=https://mock-napi.igourd.pro/other-api
  ```

- In `packages/types/global.d.ts`, add the corresponding type definition, such as:

  ```ts
  export interface IgourdAdminProAppConfigRaw {
...
- In `packages/effects/hooks/src/use-app-config.ts`, add the corresponding configuration item, such as:
  ...
  export function useAppConfig(
    ...
    // In production environment, directly use the window._IGOURD_ADMIN_PRO_APP_CONF_ global variable
    const config = isProduction
      ? window._IGOURD_ADMIN_PRO_APP_CONF_
      : (env as IgourdAdminProAppConfigRaw);
...
const defaultPreferences: Preferences = {
  app: {
    ...
    contentPaddingTop: 0,
    defaultAvatar:
      'https://unpkg.com/@igourdjs/static-source@0.1.7/source/avatar-v1.webp',
  ...
  copyright: {
    companyName: 'Igourd',
    companySiteLink: 'https://www.igourd.pro',
    date: '2024',
    enable: true,
    icp: '',
    icpLink: '',
