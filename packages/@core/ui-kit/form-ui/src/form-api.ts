...
import type { FormActions, FormSchema, IgourdFormProps } from './types';

import { isRef, toRaw } from 'vue';

import { Store } from '@igourd-core/shared/store';
import {
  bindMethods,
  createMerge,
  formatDate,
...
function getDefaultState(): IgourdFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<IgourdFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | IgourdFormProps = null;
  stateHandler: StateHandler;

  public store: Store<IgourdFormProps>;
  ...
  private prevState: null | IgourdFormProps = null;

  constructor(options: IgourdFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<IgourdFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: IgourdFormProps) => Partial<IgourdFormProps>)
      | Partial<IgourdFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<IgourdForm /> is not mounted');
    }
    return this.form;
  }
