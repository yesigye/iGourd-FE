...
import type { ValueType, IgourdButtonGroupProps } from './button';

import { computed, ref, watch } from 'vue';

import { Circle, CircleCheckBig, LoaderCircle } from '@igourd-core/icons';
import { cn, isFunction } from '@igourd-core/shared/utils';

import { objectOmit } from '@vueuse/core';

import { IgourdRenderContent } from '../render-content';
import IgourdButtonGroup from './button-group.vue';
import Button from './button.vue';

const props = withDefaults(defineProps<IgourdButtonGroupProps>(), {
  gap: 0,
  multiple: false,
  showIcon: true,
  size: 'middle',
...
<template>
  <IgourdButtonGroup
    :size="props.size"
    :gap="props.gap"
    class="igourd-check-button-group"
  >
    <Button
    ...
    >
      ...
      <slot name="option" :label="btn.label" :value="btn.value" :data="btn">
        <IgourdRenderContent :content="btn.label" />
      </slot>
    </Button>
  </IgourdButtonGroup>
</template>
<style lang="scss" scoped>
.igourd-check-button-group {
  display: flex;
  flex-wrap: wrap;
