...
export type FormItemProps = {
  addonAfter?: Component | string;
  addonBefore?: Component | string;
  asterisk?: boolean;
  bordered?: boolean;
  className?: string;
  colon?: boolean;
  extra?: string;
  feedbackIcon?: Component | string;
  feedbackLayout?: 'loose' | 'none' | 'popover' | 'terse' | (string & {});
  feedbackStatus?: 'error' | 'pending' | 'success' | 'warning' | (string & {});
  feedbackText?: Component | string;
  fullness?: boolean;
  ...
  mapProps(
    ...
    (props, field) => {
      ...
      const takeMessage = () => {
        ...
        if (field.selfErrors.length > 0) return split(field.selfErrors);
        if (field.selfWarnings.length > 0) return split(field.selfWarnings);
        if (field.selfSuccesses.length > 0) return split(field.selfSuccesses);
      };
