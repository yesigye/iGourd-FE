...
import { UploadFilled, Download as IconDownload, View as IconView, Delete as IconDelete } from '@element-plus/icons-vue'
...
export type UploadProps = {
  ...
  value?: UploadValueObject[] | string[]
  ...
  transformResponse?: (resp: any, file: any) => UploadValueObject | string | null
...
// ====== 值 <-> ElUpload 文件结构 转换 ======
const toElFiles = (val: UploadValueObject[] | string[] | undefined, valueType: ValueType) => {
  const arr = Array.isArray(val) ? val : []
  return arr.map((it: any, idx) => {
    if (valueType === 'url') {
      return { uid: `u-${idx}-${String(it)}`, name: String(it).split('/').pop() || `file-${idx}`, url: String(it), status: 'success' }
    }
    const o = it as UploadValueObject
    return { uid: o.uid ?? `u-${idx}-${o.url}`, name: o.name || (o.url?.split('/').pop() || `file-${idx}`), url: o.url, size: o.size, type: o.type, status: 'success', raw: o }
  })
}

const fromElFiles = (files: any[], valueType: ValueType): UploadValueObject[] | string[] => {
  if (valueType === 'url') {
    return files
      .filter((f) => !!f?.url)
      .map((f) => String(f.url))
...
export default connect(
  defineComponent<UploadProps>({
    ...
    setup(props, { attrs }) {
      ...
      const fileList = ref<any[]>(toElFiles(props.value as any, valueType.value))
      ...
      const onSuccess = (response: any, uploadFile: any, uploadFiles: any[]) => {
        ...
        if (typeof props.transformResponse === 'function') {
          ...
          const name =
            ...
            uploadFile?.name
          if (url) {
            mapped = valueType.value === 'url' ? url : { url, name, size: uploadFile?.size, type: uploadFile?.type }
          }
        }

        if (!mapped) {
          // 如果无法识别响应，回退为本地对象（但 url 为空，需要后端返回才可下载）
          mapped = valueType.value === 'url'
            ? (uploadFile?.url || '')
            : { url: uploadFile?.url || '', name: uploadFile?.name, size: uploadFile?.size, type: uploadFile?.type }
        }
        ...
        if (idx > -1) {
          uploadFiles[idx] = {
            ...uploadFiles[idx],
            url: valueType.value === 'url' ? String(mapped) : (mapped as UploadValueObject).url,
      ...
      // —— 只读态渲染（Preview）——
      const readPretty = computed(() => field.value.readPretty || props.disabled)

      const renderReadPretty = () => {
        const files = fileList.value || []
        if (!files.length) return h('div', { style: 'color: var(--el-text-color-placeholder);' }, '无附件')
        return h(
          'div',
          { class: 'uploadfiles-readpretty', style: 'display:flex; flex-wrap:wrap; gap:8px;' },
          files.map((f, i) =>
            isImage(f)
              ? h('div', { key: f.uid, style: 'width:96px; height:96px; border:1px solid var(--el-border-color); border-radius:6px; overflow:hidden; position:relative; cursor:pointer;' }, [
                  h('img', {
                    src: f.url,
              ...
              : h(
                  'div',
                  { key: f.uid, style: 'display:flex; align-items:center; gap:6px; border:1px solid var(--el-border-color); padding:6px 8px; border-radius:6px;' },
                  [
                    h(ElIcon, { size: 16 }, { default: () => h(UploadFilled) }),
                    h('a', { href: f.url, target: '_blank', style: 'max-width:260px; overflow:hidden; text-overflow:ellipsis; whiteSpace:"nowrap"' }, f.name || f.url),
                    h(ElButton, { text: true, onClick: () => onPreview(f) }, { default: () => [h(ElIcon, null, { default: () => h(IconView) }), ' 预览'] }),
                    h(ElButton, { text: true, onClick: () => window.open(f.url, '_blank') }, { default: () => [h(ElIcon, null, { default: () => h(IconDownload) }), ' 下载'] }),
                  ],
                ),
          ),
        )
            ...
            default: () =>
              props.drag
                ? h('div', { style: 'padding:20px; text-align:center;' }, [
                    ...
                    h('div', { style: 'margin-top:8px; color: var(--el-text-color-secondary);' }, '将文件拖拽到此处或点击上传'),
                  ])
                : h(
                    ElButton,
                    { type: 'primary' },
        ...
        h('div', { class: 'formily-x-uploadfiles' }, [
          ...
          props.previewInViewer && viewerVisible.value
            ? h(ElImageViewer as any, {
                urlList: (fileList.value || []).filter(isImage).map((f) => f.url),
