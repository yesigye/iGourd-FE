...
import { IgourdRenderContent } from '@igourd-core/shadcn-ui';
...
declare global {
  const __IGOURD_ADMIN_METADATA__: {
    authorEmail: string;
    authorName: string;
    authorUrl: string;
    buildTime: string;
...
const renderLink = (href: string, text: string) =>
  h(
    'a',
    { href, target: '_blank', class: 'igourd-link' },
    { default: () => text },
  );
...
} = __IGOURD_ADMIN_METADATA__ || {};

const igourdDescriptionItems: DescriptionItem[] = [
  {
    content: version,
    title: '版本号',
  },
  ...
  {
          content: renderLink(IGOURD_DOC_URL, '点击查看'),
    title: '文档地址',
  },
  {
          content: renderLink(IGOURD_PREVIEW_URL, '点击查看'),
    title: '预览地址',
  },
  {
          content: renderLink(IGOURD_GITHUB_URL, '点击查看'),
    title: 'Github',
  },
  {
    content: h('div', [
  ...
  <Page :title="title">
    <template #description>
      <p class="text-foreground mt-3 text-sm leading-6">
        <a :href="IGOURD_GITHUB_URL" class="igourd-link" target="_blank">
          {{ name }}
        </a>
        {{ description }}
      </p>
    ...
    <div class="card-box p-5">
      ...
      <div class="mt-4">
        <dl class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          <template v-for="item in igourdDescriptionItems" :key="item.title">
            <div class="border-border border-t px-4 py-6 sm:col-span-1 sm:px-0">
              ...
              <dd class="text-foreground mt-1 text-sm leading-6 sm:mt-2">
                <IgourdRenderContent :content="item.content" />
              </dd>
            </div>
          </template>
        </dl>
