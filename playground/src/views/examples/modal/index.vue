...
const [BaseModal, baseModalApi] = useIgourdModal({
  // 连接抽离的组件
  connectedComponent: BaseDemo,
});

const [InContentModal, inContentModalApi] = useIgourdModal({
  // 连接抽离的组件
  connectedComponent: InContentModalDemo,
});

const [AutoHeightModal, autoHeightModalApi] = useIgourdModal({
  connectedComponent: AutoHeightDemo,
});

const [DragModal, dragModalApi] = useIgourdModal({
  connectedComponent: DragDemo,
});

const [DynamicModal, dynamicModalApi] = useIgourdModal({
  connectedComponent: DynamicDemo,
});

const [SharedDataModal, sharedModalApi] = useIgourdModal({
  connectedComponent: SharedDataDemo,
});

const [FormModal, formModalApi] = useIgourdModal({
  connectedComponent: FormModalDemo,
});

const [NestedModal, nestedModalApi] = useIgourdModal({
  connectedComponent: NestedDemo,
});

const [BlurModal, blurModalApi] = useIgourdModal({
...
<template>
  ...
  >
    <template #extra>
      <DocButton path="/components/common-ui/igourd-modal" />
    </template>
    <BaseModal />
    <InContentModal />
    <AutoHeightModal />
    ...
    <Flex wrap="wrap" class="w-full" gap="10">
      ...
      <Card class="w-[300px]" title="轻量提示弹窗">
        <template #extra>
          <DocButton path="/components/common-ui/igourd-alert" />
        </template>
