...
import { ElSelect, ElOption } from '@igourd/common-ui'
import {
  computed,
  defineComponent,
  onMounted,
...
export const SearchSelect = defineComponent<SearchSelectProps>({
  ...
  setup: (props, context) => {
    ...
    const opts = computed(() =>
      uniqBy(
        (data.value?.list || []).concat(toRaw(selectOptions.value)),
        'value',
      ),
    )
    /** 选中的原始数据值 */
    const selectRawValue = computed(() => {
      if (!selectValue.value || !selectValue.value.length) return undefined
      if (selectValue.value instanceof Array) {
        return opts.value.filter(opt => selectValue.value.includes(opt.value))
      }
    ...
    watch(
      ...
      newVal => {
        if (!newVal || !newVal.length) {
          ...
          selectOptions.value = opts.value.filter(opt => newVal === opt.value)
        }
      },
      { immediate: true },
    )
