...
export function useInventoryProductFeatureList() {
  ...
  const columns: VxeGridPropTypes.Column<ProductFeatureItem>[] = [
    ...
    },
    {
      ...
      sortable: true,
      formatter: ({ cellValue }) => {
        return cellValue === 'SELECT' ? t('inventory.select') : t('inventory.input');
      }
    },
    {
  ...
  return useCrud<ProductFeatureItem, ProductFeatureParams>({
    ...
    batchOperate: true,
    service: {
      query: inventoryProductFeatureApi.getDynamicColumnList,
      create: inventoryProductFeatureApi.createDynamicColumn,
      update: inventoryProductFeatureApi.updateDynamicColumn,
      drop: inventoryProductFeatureApi.deleteDynamicColumn
    }
  });
}
