...
export function useEditCustomerDrawer() {
  const { t } = useI18n();
  ...
  const [Drawer, drawerApi] = useIgourdDrawer({
    ...
    onConfirm: async () => {
      try {
        ...
        const res = await customerApi.update((data as any).id, data);
  ...
  const openDrawer = (customerData?: any) => {
    if (customerData) {
      formAPI.setValues(customerData);
    } else {
      formAPI.reset();
    }
    drawerApi.open();
