...
// import ReportSalesDrawerFrom from '../components/report-sales-drawer.vue';

// 定义行数据类型 - 基于原有的表格数据结构
interface ReportSalesInfo {
  id: string;
...
export function useReportSalesList() {
  const { t } = useI18n();

  // 获取抽屉组件
  const [Drawer, drawerApi] = useIgourdDrawer({
    connectedComponent: ReportSalesDrawerFrom,
  });
  ...
  const columns = computed(() => [
    ...
    },
    {
      ...
      title: "{{t('reports.totalSalesAmount')}}",
      width: 160,
      align: 'left',
      sortable: true,
      // 销售总金额列需要格式化
  ...
  // Grid 事件配置
  const gridEvents: VxeGridListeners<ReportSalesInfo> = {
    cellClick: ({ row }) => {
  ...
  // Grid 选项配置
  const gridOptions: VxeGridProps<ReportSalesInfo> = {
    checkboxConfig: {
      highlight: true,
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async ({ page }, form = {}) => {
          return await reportApi.getSalesReport({
