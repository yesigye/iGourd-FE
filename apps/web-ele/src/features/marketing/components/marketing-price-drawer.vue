  ...
  <div class="marketing-price-drawer">
    ...
    <div class="drawer-content">
      <div class="creator-info">
        {{ t('marketing.creator') }}: <span>{{ userInfo?.user_model?.name }}</span>
      </div>
      <Form
        :form="form"
        :schema="formSchema"
        ...
        <template #relation_type>
          <FormItem :label="t('marketing.selectormode')">
            <Radio.Group v-model="form.values.relation_type">
              ...
              <Radio value="PRODUCT_GROUP">{{ t('marketing.productgroup') }}</Radio>
              <Radio value="PRODUCT_LABEL">{{ t('marketing.productlabel') }}</Radio>
              <Radio value="PRODUCT">{{ t('marketing.productName') }}</Radio>
            </Radio.Group>
          </FormItem>
        </template>
        <template #relation_products>
          <div v-if="form.values.relation_type && form.values.relation_type !== 'ALL'" class="relation-products">
            <el-collapse v-model="form.values.relation_type">
              <el-collapse-item :title="t(`marketing.${form.values.relation_type}`)" :name="form.values.relation_type">
                <el-table
                  :data="getTableData()"
                  stripe
                  border
                ...
                >
                  ...
                  <el-table-column :label="t('marketing.action')" width="120" align="center">
                    <template #default="scope">
                      <el-button
                        link
                        type="primary"
...
import { Form, FormItem, FormButtonGroup, Submit, Reset, Select, Option, Radio, Radio.Group } from '@formily/element-plus';
...
const handleProductChange = (val: any, row: any) => {
  if (form.values.relation_type === 'PRODUCT_GROUP') {
    const selectedGroup = groupData.value.find(group => group.id === val);
    if (selectedGroup) {
      const isAlreadySelected = selectedGroups.value.some(item => item !== row && item.id === val);
  ...
  } else if (form.values.relation_type === 'PRODUCT_LABEL') {
    const selectedLabel = labelData.value.find(label => label.id === val);
    if (selectedLabel) {
      const isAlreadySelected = selectedLabels.value.some(item => item !== row && item.id === val);
      if (isAlreadySelected) {
  ...
  } else if (form.values.relation_type === 'PRODUCT') {
    const product = productData.value.find(item => item.id === val);
    if (product) {
      const isAlreadySelected = selectedProducts.value.some(item => item !== row && item.id === val);
      if (isAlreadySelected) {
...
watch(() => props.formData, (newData) => {
  if (newData) {
    ...
    switch (newData.relation_type) {
      case 'PRODUCT_GROUP':
        if (newData.relation_product_group_list && newData.relation_product_group_list.length > 0) {
          selectedGroups.value = [...newData.relation_product_group_list];
        } else {
          selectedGroups.value = [{ id: null }];
        }
        break;
      case 'PRODUCT_LABEL':
        if (newData.relation_product_label_list && newData.relation_product_label_list.length > 0) {
          selectedLabels.value = [...newData.relation_product_label_list];
        } else {
          selectedLabels.value = [{ id: null }];
        }
        break;
      case 'PRODUCT':
        if (newData.relation_product_list && newData.relation_product_list.length > 0) {
          selectedProducts.value = [...newData.relation_product_list];
        } else {
          selectedProducts.value = [{ id: null, code: '', major_name: '' }];
        }
        break;
    }
  }
