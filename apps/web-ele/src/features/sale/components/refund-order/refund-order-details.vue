...
import { orderReturnedDetails } from '@@/sale/apis';
...
defineOptions({
  name: 'SaleOrderRefundOrderDetailsDrawer',
});
const currency = ref('');
const { t } = useI18n();
// 商品表格配置
  ...
  proxyConfig: {
    ajax: {
      query: async () => {
        ...
        return orderDetail.value.order_returned_item_model_list || [];
      },
    },
  },
};
...
const handleOrderDetail = async (order_no: string) => {
  const result = await orderReturnedDetails({ order_returned_no: order_no });
