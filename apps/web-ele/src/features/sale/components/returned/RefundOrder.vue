...
const getReturnedDetail = async () => {
  ...
  if (res.code === 'SUCCESS') {
    newOrderDetails.value = res?.data;
    createReturedInfo.value = res?.data;
    newOrderDetails.value.order_item_model_list =
      res.data.order_returned_item_model_list;
  }
};
watch(
...
const handleRefundOrder = async () => {
  ...
  try {
    ...
    if (String(res.code) === 'SUCCESS') {
      ...
      emit('paySuccess');
    } else if (res.code == 'PAYMENT_METHOD_IS_DISABLED') {
      checkPaymentMethod(res.message);
    } else {
...
const getTemplateList = async (type: string) => {
  ...
  if (String(res.code) === 'SUCCESS' && type === 'REFUND_RECEIPT') {
    const data = res?.data || [];
...
const getRefundableAmountData = async () => {
  ...
  if (res.code === 'SUCCESS') {
    refundableAmountData.value = res.data;
  }
};
      ...
      <div class="bg-bg relative w-2/3">
        ...
        <div class="refund-option absolute bottom-0 left-0 w-full">
          <div class="flex h-16 items-center justify-end bg-white">
            ...
            >
              <span class="text-white">{{ t('common.pay') }}</span>
            </ElButton>
          </div>
        </div>
      </div>
