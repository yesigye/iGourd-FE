...
import { allColumnsSelect } from '@@/sale/components/scan/const/sale.config';
import { Decimal } from 'decimal.js';

import { getSystemConfigurationDetailApi } from '#/api/common/system';
...
// 商品的输入数量
const sale_warehouse_product_stock_quantity = ref(0);
...
const handleQuantityChangeOriginal = (item: any) => {
  if (
    item.sale_warehouse_product_stock_quantity === null ||
    ...
    item.sale_warehouse_product_stock_quantity = retainDecimal8(
      decimalQuantity,
      8,
    );
  }
...
function increaseEventOriginal(row) {
  if (row.sale_warehouse_product_stock_quantity >= props.mergeGoodsList) {
    emit('update-quantity', row);
    return;
  }
  row.sale_warehouse_product_stock_quantity = retainDecimal8(
    row.sale_warehouse_product_stock_quantity + 1,
    8,
  );
  emit('update-quantity', row);
}
...
function decreaseEventOriginal(row) {
  if (row.sale_warehouse_product_stock_quantity <= 1) {
...
<template>
  <div class="scan-content" style="height: 100%; overflow-y: auto">
    ...
    >
      <ElTableColumn
        v-for="(item, index) in allColumnsSelect"
        :key="index"
        :prop="item.prop"
      ...
      >
        <template #default="{ row, column }">
          <template
            v-if="['sale_warehouse_product_stock_quantity'].includes(item.prop)"
          >
            <div class="Inum">
