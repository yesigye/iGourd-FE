  ...
  <BasicDrawer v-bind="$attrs" :title="drawerTitle" class="w-full">
    <div class="p-4">
      ...
      >
        ...
        <FormItem
          name="order_type"
          :label="t('purchase.orderType')"
          :rules="[
            { required: true, message: t('purchase.please_select_order_type') },
          ]"
        >
          <Select
            v-model:value="formData.order_type"
            :placeholder="t('purchase.please_select_order_type')"
        ...
        <FormItem
          ...
          :rules="[
            { required: true, message: t('purchase.please_select_order_date') },
          ]"
        >
          <DatePicker
            v-model:value="formData.order_date"
            :placeholder="t('purchase.please_select_order_date')"
            style="width: 100%"
          />
        </FormItem>

        <!-- 交货日期 -->
        <FormItem name="delivery_date" :label="t('purchase.deliveryDate')">
          <DatePicker
            v-model:value="formData.delivery_date"
            :placeholder="t('purchase.please_select_delivery_date')"
            style="width: 100%"
          />
        </FormItem>
...
const orderTypeOptions = [
  { label: t('purchase.orderType.purchase'), value: 'PURCHASE' },
  { label: t('purchase.orderType.return'), value: 'RETURN' },
];

// 表单数据
const formData = ref<PurchaseOrderCreateVO | PurchaseOrderModifyVO>({
  supplier_id: 0,
  order_type: 'PURCHASE',
  order_date: '',
  delivery_date: '',
...
const drawerTitle = computed(() => {
  ...
  if (type === 'add') {
    return t('purchase.addPurchaseOrder');
  } else if (type === 'edit') {
    return t('purchase.editPurchaseOrder');
  } else if (type === 'detail') {
    return t('purchase.purchaseOrderDetail');
  } else {
    return t('purchase.purchaseOrder');
  }
});
...
const loadFormData = async (id: number) => {
  try {
    const response = await getPurchaseOrderDetailApi({
      order_id: id,
...
const handleSubmit = async () => {
  try {
    ...
    if (drawerData.value?.type === 'add') {
      await createPurchaseOrderApi(submitData as PurchaseOrderCreateVO);
    } else if (drawerData.value?.type === 'edit' && drawerData.value?.id) {
      await updatePurchaseOrderApi({
        ...submitData,
        order_id: drawerData.value.id,
      } as PurchaseOrderModifyVO);
    }
