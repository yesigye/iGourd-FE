...
// 对象字段映射到数组对象字段
export function filterAndUpdateDynamicFields(sourceObject, dynamicLabelList) {
  return dynamicLabelList.map((item) => {
...
// // 根据一个商户id查询商户所在国家货币符号;
export function getMerchantInfo() {
  const useStore = useUserStore();
  const { currencySymbol, merchantInfo, merchantId } = useStore;
  return {
    getCurrency_symbol: currencySymbol,
    getCurrency_code: merchantInfo?.basic_currency_code,
    merchant_id: merchantId,
    merchant_name: merchantInfo?.merchant_name,
  };
}

export const dynamicStyle = (number) => {
...
export const formatNumber = (
  ...
  try {
    ...
    if (!showDecimalsAlways && decimalNum.isInteger()) {
      return decimalNum.toString().replaceAll(/\B(?=(\d{3})+(?!\d))/g, ',');
    }
